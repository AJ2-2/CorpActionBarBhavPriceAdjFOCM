...
int main(int argc,char *argv[])
{
	...
	if(f.is_open())
    {
		...
		while(getline(f,str))
		{
			...
			{
				if(v[0]=="SYMBOL") {outfile<<v[0]<<","<<v[1]<<","<<v[2]<<","<<v[3]<<","<<v[4]<<","<<v[5]<<","<<v[6]<<","<<v[7]<<","<<v[8]<<","<<v[9]<<","<<v[10]<<","<<v[11]<<","<<v[12]<<endl;continue;} 
				adjusted_action_data(v,date);					// skipping header!
				outfile<<std::fixed<<std::setprecision(2)<<v[0]<<","<<v[1]<<","<<stod(v[2])<<","<<stod(v[3])<<","<<stod(v[4])<<","<<stod(v[5])<<","<<stod(v[6])<<","<<v[7]<<","<<v[8]<<","<<v[9]<<","<<v[10]<<","<<v[11]<<","<<v[12]<<endl;
			}
		}
	}
